...
class Compute_resource_form(forms.Form):
    name = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Name", max_length=10)
    ip_address = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="IP Address")
    root_password = forms.CharField(widget=forms.PasswordInput(attrs={'size': '30'}), label="Root Password",
                                    max_length=20)


class Profile_form(forms.Form):
    profile_name = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Profile Name", max_length=10)
...
class Create_host_form(forms.Form):
    vm_name = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Name")
    vm_os = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Operating System")
    select_vm_profile = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Profile")
    select_compute = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Compute Resource")
    password = forms.CharField(widget=forms.TextInput(attrs={'size': '30'}), label="Root Password")
