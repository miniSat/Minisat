...
def get_compute_details(ip_address):
    ...
    tmp_dict = {}
    tmp_dict['total'], tmp_dict['used'] = ram_details[1], ram_details[2]
    tmp_dict['ram-percent-used'] = str(round(float(ram_details[2]) / float(ram_details[1]) * 100, 2))
    ...
    if tmp_dict['used'].endswith('k'):
        tmp_dict['used'] = str(round(float(tmp_dict['used'][:-1]) / 1024, 2))
    elif tmp_dict['used'].endswith('G'):
        tmp_dict['used'] = tmp_dict['used'][:-1]
